...
def main(rp, hp, md, dt):
    ...
    for i in range(hp.n_epochs + 1):
        ...
        local_loss = train(don_model, x_branch, x_trunk, y_true, i == 0)
        ...
        if i % hp.interval == 0:
            ...
            if hvd.rank() == 0:
                ...
                logger.info(
                    f"Rank:{hvd.rank()} epoch:{i}  Train Loss:{train_loss:.3e}, Val Loss:{valid_loss:.3e}, elapsed time:{int(time.time() - begin_time)} s"
                )
